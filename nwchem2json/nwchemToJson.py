...
class nwchemToJson:
    ...
    def convert(self, streamIn):
        ...
        return json.dumps(
            {
                "simulation": {
                    "simulationEnvironment": self.simulationEnv,
    ...
    def genFuncForMol(self):
        def generateFunctions(funcName, spherical):
            ...
            def funcGen(lValue, funcName):
                for comp in xyz:
                    if xyz.index(comp) >= (
                        xyz.index(funcName[-1:]) if len(funcName) > 1 else -1
                    ):
                        if lValue > 1:
            ...
            for basis in self.basis.basis["basisFunctions"]:
                if (
                    ...
                    for contr in basis["basisSetContraction"]:
                        ...
                        for x in generateFunctions(
                            contr["basisSetShellType"], spherical
                        ):
                    ...
                    for contr in basis["basisSetContraction"]:
                        if contr["basisSetShellType"] == "s":
                            ...
                            if spherical:
                                ...
                                for i in range(0, 9):
                                    coefficients.append(
                                        coefficientsIn[functionCount + i]
                                    )
                                functionCount += 9
        return coefficients
        ...
        while line:
            ...
            for envKey in envList.keys():
                if line.find(envKey) >= 0:
                    self.simulationEnv[envList.get(envKey)] = (
    ...
    def readTaskTimes(self, line):
        vars = line.split()
        return {
            "cpuTime": float(vars[3].rstrip("s")),
            "wallTime": float(vars[5].rstrip("s")),
            "units": "second",
        }
    ...
    def readScfDft(self, line, streamIn):
        ...
        def openShell(line):
            val = line.split()[3]
            self.calcSetup["numberOfElectrons"] = int(val) + int(
                self.calcSetup["numberOfElectrons"]
            )
        ...
        def alphaElec(line):
            self.calcSetup["numberOfElectrons"] = int(line.split()[3]) + int(
                self.calcSetup["numberOfElectrons"]
            )

        def betaElec(line):
            val = line.split()[3]
            self.calcSetup["molecularSpinMultiplicity"] = (
                abs(int(val) - int(self.calcSetup["numberOfElectrons"])) / 2 + 1
            )
            self.calcSetup["numberOfElectrons"] = int(val) + int(
                self.calcSetup["numberOfElectrons"]
            )
        ...
        def totalEn(line):
            self.calcRes["totalEnergy"] = {
                "value": float(line.split()[4].replace("D", "E")),
                "units": "Hartree",
            }

        def oneEn(line):
            self.calcRes["oneElectronEnergy"] = {
                "value": float(line.split()[3].replace("D", "E")),
                "units": "Hartree",
            }
        ...
        def twoEn(line):
            self.calcRes["twoElectronEnergy"] = {
                "value": float(line.split()[3].replace("D", "E")),
                "units": "Hartree",
            }
        ...
        def coulEn(line):
            self.calcRes["coulombEnergy"] = {
                "value": float(line.split()[3].replace("D", "E")),
                "units": "Hartree",
            }

        def xcEn(line):
            self.calcRes["exchangeCorrelationEnergy"] = {
                "value": float(line.split()[3].replace("D", "E")),
                "units": "Hartree",
            }

        def xEn(line):
            self.calcRes["exchangeEnergy"] = {
                "value": float(line.split()[3].replace("D", "E")),
                "units": "Hartree",
            }

        def cEn(line):
            self.calcRes["correlationEnergy"] = {
                "value": float(line.split()[3].replace("D", "E")),
                "units": "Hartree",
            }

        def s2Val(line):
            self.calcRes["s2ExpectationValue"] = {
                "value": float(line.split()[2]),
                "units": "none",
            }

        def szVal(line):
            self.calcRes["szExpectationValue"] = {
                "value": float(line.split()[2]),
                "units": "none",
            }
            ...
            while line.find("Grid Info") < 0 and len(line.split()) > 1:
                if line.find("Method") >= 0:
                    ...
                    if len(vars[2].split()) > 1:
                        ...
                        exchange.append(
                            {
                                "correlationTermName": vars[0] + vars[1],
                    ...
                    if len(vars[2].split()) > 1:
                        ...
                        exchange.append(
                            {
                                "exchangeTermName": vars[0] + vars[1],
                ...
                while line:
                    if line.find("Vector") >= 0:
                        ...
                        orbital["id"] = (
                            ...
                            + str(vars[1])
                            + ".Mol."
                            + str(self.molecule.molCount)
                        )
                        orbital["orbitalNumber"] = int(vars[1])
                        orbital["orbitalEnergy"] = {
                            "value": float(vars[5].replace("D", "E")),
                            "units": "Hartree",
                        }
                        ...
                        while True:
                            ...
                            if len(vars) > 6:
                                if len(vars) == 10 or (
                                    len(vars) == 11 and vars[9] in ["f", "g", "h", "i"]
                                ):
                        ...
                        orbital["moCoefficients"] = self.reorderFunctionsInMO(
                            coefficients
                        )
        ...
        while line:
            if line.find("Task  times") >= 0 and not self.subTask:
                ...
                line = streamIn.readline()
                self.calcRes["totalEnergy"] = {
                    "value": streamIn.readline().split("=")[1],
                    "units": "Hartree",
                }
                break
            elif (
                line.find("Module") >= 0
                or line.find("Parallel integral file") >= 0
                or line.find("Line search") >= 0
                or line.find("Saving state") >= 0
            ) and self.subTask:
                break
    ...
    def readTddft(self, line, streamIn):
        ...
        while line.find("TDDFT Info") < 0:
            line = streamIn.readline()
        line = streamIn.readline()
        self.calcTask["calculationType"] = (
            streamIn.readline().split(":")[1].rstrip("\n")
        )
        self.calcTask["waveFunctionType"] = (
            streamIn.readline().split(":")[1].lstrip(" ").rstrip("\n")
        )
        unrestricted = self.calcTask["waveFunctionType"].startswith("Unrestricted")
        while line.find("Ground state") < 0:
            line = streamIn.readline()
        vars = line.split()
        self.calcRes["groundState"] = {
            "groundStateEnergy": {"value": float(vars[3]), "units": "Hartree"},
            "groundStateSymmetry": vars[2],
        }
        if unrestricted:
            self.calcRes["groundState"].update(
                {
                    "s2ExpectationValue": {
                        "value": float(line.split()[2]),
                        "units": "none",
        ...
        if singlets or unrestricted:
            ...
            while line:
                if line.find("Root  ") >= 0:
                    vars = line.split()
                    state = {
                        "excitedState": vars[1] + exstate,
                        "excitedStateSymmetry": vars[3],
                        "excitationEnergy": {
                            "value": float(vars[4]),
                            "units": "Hartree",
                        },
                    ...
                    if unrestricted:
                        state.update(
                            {
                                "s2ExpectationValue": {
                                    "value": float(streamIn.readline().split()[2]),
                                    "units": "none",
                                }
                            }
                        )
                    ...
                    state.update(
                        {
                            "dipoleOscillatorStrength": {
                                "value": float(streamIn.readline().split()[3]),
                                "units": "none",
                            }
                        }
                    )
        ...
        if triplets:
            ...
            while line:
                if line.find("Root") >= 0:
                    ...
                    state.update(
                        {"dipoleOscillatorStrength": {"value": 0.0, "units": "none"}}
                    )
                    tddftStates.append(state)
                elif line.find("Target root") >= 0:
                    line = streamIn.readline()
    ...
    def readFreq(self, line, streamIn):
        ...
        def tempFreq(line, streamIn):
            self.calcRes["temperatureVibrations"] = {
                "value": float(line.split()[2].rstrip("K")),
                "units": "Kelvin",
            }
        ...
        def zpeBlock(line, streamIn):
            self.calcRes["zeroPointEnergyCorrection"] = {
                "value": float(line.split()[8]),
                "units": "Hartree",
            }
            self.calcRes["thermalEnergyCorrection"] = {
                "value": float(streamIn.readline().split()[8]),
                "units": "Hartree",
            }
            self.calcRes["thermalEnthalpyCorrection"] = {
                "value": float(streamIn.readline().split()[8]),
                "units": "Hartree",
            }
        ...
        def entBlock(line, streamIn):
            ...
            vars = streamIn.readline().split()
            if len(vars) == 8:
                entropy["translationalEntropyContribution"] = {
                    "value": float(vars[3]),
                    "units": vars[4],
                    "molecularWeight": vars[7].rstrip(")").lstrip("="),
                }
            else:
                entropy["translationalEntropyContribution"] = {
                    "value": float(vars[3]),
                    "units": vars[4],
                    "molecularWeight": vars[8].rstrip(")"),
                }
            vars = streamIn.readline().split()
            entropy["rotationalEntropyContribution"] = {
                "value": float(vars[3]),
                "units": vars[4],
                "symmetryNumber": vars[8].rstrip(")"),
            }
            vars = streamIn.readline().split()
            entropy["vibrationalEntropyContribution"] = {
                "value": float(vars[3]),
                "units": vars[4],
            }
        ...
        while maxMod < (numberOfModes - 1):
            ...
            for i in range(minMod, maxMod):
                ...
                if abs(float(freq[i])) > 0.0:
                    ...
                    normalModes.append(
                        {
                            "id": "normalMode." + str(i + 1),
                            "normalModeFrequency": {
                                "value": float(freq[i]),
                                "units": "cm-1",
                            },
                            "normalModeInfraRedIntensity": {
                                "value": float(inten[i]),
                                "units": "(debye/angs)**2",
                            },
    ...
    def readProp(self, line, streamIn):
        ...
        def propDip(streamIn):
            ...
            for _ in range(3):
                line = streamIn.readline()
            vars = streamIn.readline().split()
            dipProp["expansionPoint"] = {
                "value": [float(vars[2]), float(vars[5]), float(vars[8])],
                "units": "atomic units",
            }
            line = streamIn.readline()
            dipProp["totalMoment"] = {
            ...
            for i in range(3):
                dipProp[myList[i]] = {
                    "value": float(streamIn.readline().split()[1]),
                    "units": "atomic units",
                }
        ...
        def propQuad(streamIn):
            ...
            for _ in range(3):
                line = streamIn.readline()
            vars = streamIn.readline().split()
            quadProp["expansionPoint"] = {
                "value": [float(vars[2]), float(vars[5]), float(vars[8])],
                "units": "atomic units",
            }
            line = streamIn.readline()
            quadProp["diamagneticSusceptibility"] = {
                "value": float(streamIn.readline().split()[4]),
                "units": "atomic units",
            }
            for _ in range(6):
                line = streamIn.readline()
        ...
        def propEFG(streamIn):
            ...
            for atomNum in range(1, self.molecule.atomCount + 1):
                ...
                for _ in range(4):
                    line = streamIn.readline()
                vars = streamIn.readline().split()
                efgProp["efgPrincipalComponents"] = {
                    "values": [float(vars[0]), float(vars[1]), float(vars[2])],
                    "units": "atomic units",
                }
            ...
            while line:
                if line.find("Atom:") >= 0:
                    ...
                    for a in range(3):
                        ...
                        for _ in range(2):
                            line = streamIn.readline()
                    chemshield["isotropicShielding"] = {
                        "value": float(line.split()[2]),
                        "units": "ppm",
                    }
                    chemshield["shieldingAnisotropy"] = {
                        "value": float(streamIn.readline().split()[2]),
                        "units": "ppm",
                    }
                    for _ in range(3):
                    ...
                    chemshield["shieldingPrincipalComponents"] = {
                    ...
                    chemshield["shieldingProjectionVectors"] = {
                        "vectorValues": value,
                        "units": "none",
                    }
        ...
        def propSpinSpin(streamIn):
            myList = [
                "Fermi Contact Term",
                "Spin-Dipole Term",
                "Fermi Contact - Spin-Dipole Cross Term",
            ...
            while line:
                if line.find("Atom") >= 0:
                    ...
                    if atom1 not in atomList:
                        ...
                        spinspin.append(
                            {
                                "atomicWeight": float(vars[2].split("-")[0]),
                                "spinSpinCouplingPairs": [],
                            }
                        )
                    if atom2 not in atomList:
                        ...
                        spinspin.append(
                            {
                                "atomicWeight": float(vars[6].split("-")[0]),
                                "spinSpinCouplingPairs": [],
                            }
                        )
                    coupledAtom1 = {
                        "atom": "Atom." + str(atom2) + ".Mol." + str(molCount),
                        "atomicWeight": float(vars[6].split("-")[0]),
                    }
                    coupledAtom2 = {
                        "atom": "Atom." + str(atom1) + ".Mol." + str(molCount),
                        "atomicWeight": float(vars[2].split("-")[0]),
                    }
                    line = streamIn.readline()
                    vars = streamIn.readline().split()
                    ...
                    for i in range(6):
                        ...
                        spinpair1[myList[i]] = {
                            "tensorValues": value,
                            "isotropicValue": float(iso),
                            "units": "Hertz",
                        }
                        for _ in range(2):
                            line = streamIn.readline()
                    spinpair1["Isotropic Spin-Spin Coupling"] = {
                        "value": float(streamIn.readline().split()[4]),
                        "units": "Hertz",
                    }
                    spinpair2 = spinpair1
                    ...
                    spinspin[atomList.index(atom1)].update(
                        {"spinSpinCouplingPairs": spinpair1}
                    )
                    spinspin[atomList.index(atom2)].update(
                        {"spinSpinCouplingPairs": spinpair2}
                    )
                    linesSkipped = 0
                ...
                for i in range(len(atomList)):
                    molProps[int(atomList[i])].update(
                        {"spinSpinCoupling": spinspin[i - 1]}
                    )
                line = streamIn.readline()
            return line
    ...
    def readTce(self, line, streamIn):
        ...
        self.calcSetup["numberOfAlphaElectrons"] = int(
            streamIn.readline().split(":")[1]
        )
        self.calcSetup["numberOfBetaElectrons"] = int(streamIn.readline().split(":")[1])
        for _ in range(3):
            line = streamIn.readline()
        self.calcSetup["numberOfFrozenCoreElectrons"] = int(
            streamIn.readline().split(":")[1]
        )
        line = streamIn.readline()
        self.calcSetup["numberOfFrozenVirtualOrbitals"] = int(
            streamIn.readline().split(":")[1]
        )
        self.calcSetup["molecularSpinMultiplicity"] = int(
            streamIn.readline().split(":")[1]
        )
        ...
        if perturbative:
            ...
            self.calcSetup["waveFunctionTheory"] += (
                " with "
                + streamIn.readline().split(":")[1]
        ...
        while line:
            if line.find("correlation energy") >= 0:
                ...
                self.calcRes["correlationEnergy"].append(
                    {"theory": theory, "value": float(vars[1]), "units": "Hartree"}
                )
            elif line.find("correction energy") >= 0:
                vars = line.split("=")
                theory = vars[0].split("correlation")[0]
                self.calcRes["perturbativeCorrectionEnergy"].append(
                    {"theory": theory, "value": float(vars[1]), "units": "Hartree"}
                )
            elif line.find("total energy") >= 0:
                vars = line.split("=")
                theory = vars[0].split("correlation")[0]
                self.calcRes["totalEnergy"].append(
                    {"theory": theory, "value": float(vars[1]), "units": "Hartree"}
                )
            elif line.find("Task  times") >= 0:
                self.calcTask["calculationTime"] = self.readTaskTimes(line)
...
class basisObj:
            ...
            if line.find('Summary of "ao') >= 0:
                ...
                while line:
                    ...
                    if len(vars) == 0:
                        ...
                        self.basis["basisFunctions"][count].update(
                            {"basisSetName": vars[1]}
                        )
                        count += 1
                    line = streamIn.readline()
    ...
    def readEcp(self, line, streamIn):
        ...
        while line:
            if (
                line.find("Module") >= 0
                or line.find("library") >= 0
                or line.find("NWChem") >= 0
                or emptyLine > 1
            ):
                break
            vars = streamIn.readline().split()
            atomLab, atomName, elec = (
                vars[0],
                vars[1].replace("(", "").replace(")", ""),
                vars[3],
            )
            atomBasCount += 1
            ecpAtom = {}
